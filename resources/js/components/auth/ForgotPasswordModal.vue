            ...
            <v-card-text>
                <form method="POST" name="forgotPassword" role="form" @submit.prevent="onSubmit">
                    ...
                    <v-text-field
                            ...
                            ></v-text-field>
                    <v-btn class="btn btn-primary mt-4" :loading="isSubmitting" type="submit">{{$getConst('BTN_SUBMIT')}}</v-btn>
                    <v-btn color="" class="btn btn-grey ml-3 mt-4" @click="onCancel()">{{ $getConst('BTN_CANCEL') }}</v-btn>
                </form>
            </v-card-text>
        </v-card>
